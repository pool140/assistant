...
object CommandStore {
    ...
    fun getWakeWord(ctx: Context): String {
        val value = prefs(ctx).getString(KEY_WAKE_WORD, null)
        // v3 used "افتحلي" as the wake word. Migrate that old default to the
        // dedicated assistant wake word without overwriting a custom choice.
        if (value == null || value == "افتحلي") return "يا مساعد"
        return value
    }
    ...
    fun setStartOnBoot(ctx: Context, enabled: Boolean) {
        prefs(ctx).edit().putBoolean("start_on_boot", enabled).apply()
    }

    fun isStartOnBootEnabled(ctx: Context): Boolean =
        prefs(ctx).getBoolean("start_on_boot", false)
    ...
    fun getApps(ctx: Context): List<AppShortcut> {
        ...
        for (i in 0 until arr.length()) {
            ...
            list.add(AppShortcut(o.getString("label"), o.getString("packageName"), if (o.has("aliases")) { val a=o.getJSONArray("aliases"); List(a.length()) { i -> a.getString(i) } } else emptyList()))
        }
        return list
    }
    ...
    fun saveApps(ctx: Context, apps: List<AppShortcut>) {
        ...
        apps.forEach {
            ...
            o.put("aliases", JSONArray(it.aliases))
            arr.put(o)
        }
    ...
    private fun defaultApps(): List<AppShortcut> = listOf(
        AppShortcut("فيسبوك", "com.facebook.katana", listOf("فيس", "الفيس", "الفيسبوك")),
        AppShortcut("شات جي بي تي", "com.openai.chatgpt", listOf("ChatGPT", "شات جي بي تي", "شات جيبيتي", "جي بي تي")),
        AppShortcut("كلود", "com.anthropic.claude", listOf("Claude")),
        AppShortcut("واتساب", "com.whatsapp", listOf("واتس", "الواتساب")),
        AppShortcut("يوتيوب", "com.google.android.youtube", listOf("YouTube", "اليوتيوب"))
    )
